...
DWORD WINAPI ListenThreadFunc(LPVOID pParam)
{
	...
	//将socket设置为非阻塞
	u_long mode = 1;
	if (ioctlsocket(pchatroom->m_ListenSock, FIONBIO, &mode) == SOCKET_ERROR)
	{
	...
	//设置tcp_nodelay
	char flag = 1;
	if (setsockopt(pchatroom->m_ListenSock, IPPROTO_TCP, TCP_NODELAY, &flag, sizeof(flag)) == SOCKET_ERROR)
	{
	...
	while (!pchatroom->bShutDown)
	{
		if (Socket_Select(pchatroom->m_ListenSock, 100, true))
		{
			...
			//将socket设置为非阻塞
			u_long mode = 1;
			if (ioctlsocket(AcceptSock, FIONBIO, &mode) == SOCKET_ERROR)
			{
				AfxMessageBox(_T("socket设置为non_blocking失败！"));
				closesocket(AcceptSock);
			...
			//设置tcp_nodelay
			int flag = 1;
			if (setsockopt(AcceptSock, IPPROTO_TCP, TCP_NODELAY, (char*)&flag, sizeof(flag)) == SOCKET_ERROR)
			{
				AfxMessageBox(_T("socket设置TCP_NOCELAY失败！"));
				closesocket(AcceptSock);
				AcceptSock = INVALID_SOCKET;
...
DWORD WINAPI ClientThreadProc(LPVOID pParam)
{
	...
	{
		if (Socket_Select(m_ClientItem.m_Socket, 30, true))
		{
			...
			{
				//wchar_t* wp = new wchar_t[MAX_BUF_SIZE];
				//MultiByteToWideChar(CP_ACP, 0, szbuf, MAX_BUF_SIZE, wp, MAX_BUF_SIZE);
				//strMsg = _T("客户端：") + m_ClientItem.m_strIP + _T("<<") + wp;
