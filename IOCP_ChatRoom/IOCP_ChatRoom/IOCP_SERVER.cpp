...
bool CIOCPModel::_DoRecv(PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext)
{
	...
	INT_PTR temp = m_arrayClientContext.GetCount();
	for (INT_PTR i = 0; i < temp; ++i)
	{
		if (m_arrayClientContext.GetAt(i)->m_Socket != pIoContext->m_sockAccept)
		{
			if (send(m_arrayClientContext.GetAt(i)->m_Socket, static_cast<char*>(static_cast<void*>(pIoContext->m_wsaBuf.buf)), _tcslen((wchar_t*)pIoContext->m_wsaBuf.buf) * sizeof(TCHAR), 0) == SOCKET_ERROR)
			{
				continue;
			}
		}
...
void CIOCPModel::_ShowMessage(const CString szFormat, ...) const
{
	...
	// 在主界面中显示
	CChatRoomDlg* pMain = (CChatRoomDlg*)m_pMain;
	if (m_pMain != NULL)
	{
